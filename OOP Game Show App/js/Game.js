...
class Game {
    ...
    removeLife(){
        ...
        hearts[i].getChild().src='images/lostHeart.png';
    ...
    handleInteraction(e){
        ...
        if(this.phrases.checkLetter(e.target.value)){
            //add the chosen CSS class to the selected letter's keybpard button
            e.target.classList.add('chosen');
            this.phrase.showMatchedLetter(e.target.value);
    ...
    checkForWin(){
        ...
        boardLetters.map((l) => {
            if(l.classList.contains('hide')) return false;
        });
        return true;
    }
